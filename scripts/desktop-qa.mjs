...
import { captureScreenshotWithRetry, createCdpCaller, createEvidenceCustody } from "./desktop-qa-support.mjs";

const root = resolve(new URL("..", import.meta.url).pathname);
const outputIndex = process.argv.indexOf("--output");
if (outputIndex !== -1 && !process.argv[outputIndex + 1]) throw new Error("--output requires a directory");
const custody = await createEvidenceCustody({
  env: process.env,
  outputDirectory: outputIndex === -1 ? undefined : process.argv[outputIndex + 1],
  temporaryRoot: tmpdir(),
});
const evidence = custody.directory;
const port = 4179;
...
try {
  ...
  const call=createCdpCaller(socket,{timeoutMs:30000});
  ...
  async function screenshot(path){const bytes=await captureScreenshotWithRetry(()=>call("Page.captureScreenshot",{format:"png",fromSurface:true,captureBeyondViewport:false},{timeoutMs:30000}),{attempts:2,settle:async()=>{await call("Runtime.evaluate",{expression:"document.readyState === 'complete'",returnByValue:true});await wait(150);}});await writeFile(path,bytes);}
  ...
  const contactStyle=`<style>body{margin:0;background:#050607;color:#eee;font:14px system-ui}main{display:grid;grid-template-columns:repeat(3,456px);gap:18px;padding:18px}figure{margin:0}img{display:block;width:456px;height:285px;object-fit:cover;object-position:top;border:1px solid #333}figcaption{padding:6px 0 0}</style>`;
  const contact=`<!doctype html>${contactStyle}<main>${["goal","plan","fail","pass","completion","accepted"].map(state=>`<figure><img src="./${state}-1440x900.png"><figcaption>${state}</figcaption></figure>`).join("")}</main>`;
  await writeFile(join(evidence,"contact-sheet.html"),contact);
  const embedded=await Promise.all(["goal","plan","fail","pass","completion","accepted"].map(async state=>`<figure><img src="data:image/png;base64,${(await readFile(join(evidence,`${state}-1440x900.png`))).toString("base64")}"><figcaption>${state}</figcaption></figure>`));
  await navigate(`data:text/html;base64,${Buffer.from(`<!doctype html>${contactStyle}<main>${embedded.join("")}</main>`).toString("base64")}`,1440,660,false);await screenshot(join(evidence,"contact-sheet.png"));
  socket.close();
  if(failures.length)throw new Error(`Desktop QA failed ${failures.length} rows`);
  console.log(JSON.stringify({states:states.length,viewportRows:measurements.length,keyboardSteps:keyboardSequence.length,screenshots:7,failures:0,evidenceExported:!custody.temporary,evidence:custody.temporary?"temporary custody (cleaned)":evidence},null,2));
} finally {
  server?.kill("SIGTERM");chrome?.kill("SIGTERM");await wait(100);for(const child of [server,chrome].filter(Boolean))if(child.exitCode===null)child.kill("SIGKILL");for(let attempt=0;attempt<5;attempt+=1){try{await rm(profile,{recursive:true,force:true});break;}catch(error){if(attempt===4)throw error;await wait(100);}}await custody.cleanup();
}
