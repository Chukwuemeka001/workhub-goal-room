...
test("installed UI automatically renders a current exact-PR handoff with safe navigation and zero effect authority",async()=>{const source=await readFile(new URL("./ui.js",import.meta.url),"utf8");for(const text of ["/api/workflow-preview","aca-exact-pr-handoff/v1","exact-pr-handoff-v1","Exact PR handoff · read-only","NOT_LOCAL_DIRTY_SNAPSHOT","Open PR #","Point-in-time; not monitored","Effect: NOT_ATTEMPTED","Effect capability: ABSENT","The PR may have moved since observation","sourceEnvelopeDigest"])assert.ok(source.includes(text),text);assert.match(source,/rel\s*=\s*"noopener noreferrer"/);assert.match(source,/target\s*=\s*"_blank"/);assert.match(source,/https:\/\/github\.com/);assert.doesNotMatch(source,/publish check|create check|write back|approve|merge pull|rerun workflow/i);});
...
test("workflow preview is empty-body protected, effect-free and bound only to the current settled generation",async t=>{
 const local={schema:"agent-change-assurance/connected-v3",authority:"NONE",repository:{snapshotDigest:"9".repeat(64),parentCommit:"1".repeat(40),baseCommit:"2".repeat(40)},evaluatorSnapshot:{evidence:[]}};
 const external={schema:"github-external-observation/v1",authority:"NONE",subject:"PARENT_COMMIT_ONLY"},v4={schema:"agent-change-assurance/connected-v4",authority:"NONE",envelopeDigest:"a".repeat(64),local,external};
 const handoff={schema:"aca-exact-pr-handoff/v1",authority:"NONE",effect:"NOT_ATTEMPTED",effectCapability:"ABSENT",sourceEnvelopeDigest:v4.envelopeDigest,handoffDigest:"b".repeat(64)};
 let githubCalls=0,projectCalls=0,entered;const secondEntered=new Promise(resolve=>{entered=resolve});
 const service=await startFixtureQualificationController({
  localEngine:{observe:async()=>local},
  githubEngine:{observe:async({signal})=>{githubCalls++;if(githubCalls===1)return external;entered();await new Promise(resolve=>signal.addEventListener("abort",resolve,{once:true}));throw Object.assign(new Error("CANCELLED"),{code:"CANCELLED"})}},
  compose:()=>v4,
  project:value=>{projectCalls++;assert.equal(value,v4);return handoff},
 ...
 let response=await request(service.authority,"/api/workflow-preview",{method:"POST",headers,body:""});assert.equal(response.status,409);assert.deepEqual(await response.json(),{code:"WORKFLOW_PREVIEW_UNAVAILABLE",authority:"NONE"});
 response=await request(service.authority,"/api/observe",{method:"POST",headers,body:""});assert.equal(response.status,200);assert.equal((await response.json()).schema,"agent-change-assurance/connected-v4");assert.equal(projectCalls,1);assert.equal(githubCalls,1);
 response=await request(service.authority,"/api/workflow-preview",{method:"POST",headers,body:""});assert.equal(response.status,200);assert.deepEqual(await response.json(),handoff);assert.equal(response.headers.get("x-workhub-aca-admission"),"exact-pr-handoff-v1");assert.equal(githubCalls,1,"preview must make no provider observation");
 response=await request(service.authority,"/api/workflow-preview",{method:"POST",headers,body:"{}"});assert.equal(response.status,400);assert.equal(githubCalls,1);
 const pending=request(service.authority,"/api/observe",{method:"POST",headers,body:""});await secondEntered;
 response=await request(service.authority,"/api/workflow-preview",{method:"POST",headers,body:""});assert.equal(response.status,409);assert.deepEqual(await response.json(),{code:"WORKFLOW_PREVIEW_UNAVAILABLE",authority:"NONE"});
 const cancelled=await request(service.authority,"/api/cancel",{method:"POST",headers,body:""});assert.equal(cancelled.status,200);assert.equal((await pending).status,500);
 response=await request(service.authority,"/api/workflow-preview",{method:"POST",headers,body:""});assert.equal(response.status,409);assert.equal(projectCalls,1,"cancelled generation must not project");
});

test("production projector accepts only its exact admitted v4 object and retained trusted tuple",async t=>{
 ...
 const external={schema:"github-external-observation/v1",authority:"NONE",subject:"PARENT_COMMIT_ONLY"},tuple=Object.freeze({provider:"github-rest-v1",apiVersion:"2022-11-28",owner:"nousresearch",repository:"hermes-agent",pullNumber:7}),v4=Object.freeze({schema:"agent-change-assurance/connected-v4",authority:"NONE",envelopeDigest:"a".repeat(64)}),handoff=Object.freeze({schema:"aca-exact-pr-handoff/v1",authority:"NONE",sourceEnvelopeDigest:v4.envelopeDigest,handoffDigest:"b".repeat(64)});
 let observations=0,creates=0,admits=0;
 const harness=await productionHarness(t,{github,adapters:{
  ...
  createGithubEngine:()=>({observe:async()=>{observations++;return external},trustedTuple:value=>{assert.equal(value,external);return tuple}}),
  createConnectedV4Envelope:input=>{assert.equal(input.external,external);assert.equal(input.github,tuple);return v4},admitConnectedV4Envelope:(value,binding)=>{assert.equal(value,v4);assert.equal(binding,tuple);return{valid:true,envelope:v4}},
  createExactPrHandoff:input=>{creates++;assert.equal(input.connected,v4);assert.equal(input.github,tuple);return handoff},admitExactPrHandoff:(value,expected)=>{admits++;assert.equal(value,handoff);assert.equal(expected.connected,v4);assert.equal(expected.github,tuple);return{valid:true,handoff,authority:"NONE"}},
 }});
 const service=harness.service,cap=/data-session="([0-9a-f]{64})"/.exec(await(await request(service.authority,"/")).text())[1],headers={Origin:`http://${service.authority}`,"Content-Type":"application/json","X-WorkHub-ACA-Session":cap};
 let response=await request(service.authority,"/api/observe",{method:"POST",headers,body:""});assert.equal(response.status,200);assert.equal((await response.json()).envelopeDigest,v4.envelopeDigest);
 response=await request(service.authority,"/api/workflow-preview",{method:"POST",headers,body:""});assert.equal(response.status,200);assert.equal((await response.json()).handoffDigest,handoff.handoffDigest);assert.equal(observations,1);assert.equal(creates,1);assert.equal(admits,1);
});

test("teardown aborts and invalidates an active generation before late completion can project",async t=>{
 const local={schema:"agent-change-assurance/connected-v3",authority:"NONE",repository:{snapshotDigest:"9".repeat(64),parentCommit:"1".repeat(40),baseCommit:"2".repeat(40)},evaluatorSnapshot:{evidence:[]}},external={schema:"github-external-observation/v1",authority:"NONE"},v4={schema:"agent-change-assurance/connected-v4",authority:"NONE",envelopeDigest:"a".repeat(64),local,external};
 let signal,release,projectCalls=0,externalCalls=0,entered;const atLocal=new Promise(resolve=>{entered=resolve});
 const service=await startFixtureQualificationController({localEngine:{observe:async stage=>{signal=stage.signal;entered();await new Promise(resolve=>{release=resolve});return local}},githubEngine:{observe:async()=>{externalCalls++;return external}},compose:()=>v4,project:()=>{projectCalls++;return{schema:"aca-exact-pr-handoff/v1"}}});t.after(()=>service.close());
 const html=await(await request(service.authority,"/")).text(),cap=/data-session="([0-9a-f]{64})"/.exec(html)[1],headers={Origin:`http://${service.authority}`,"Content-Type":"application/json","X-WorkHub-ACA-Session":cap},pending=request(service.authority,"/api/observe",{method:"POST",headers,body:""});await atLocal;
 const closing=service.close(),secondClosing=service.close();let secondSettled=false;secondClosing.then(()=>{secondSettled=true});const abortedAtClose=signal.aborted;await new Promise(resolve=>setTimeout(resolve,20));const secondSettledBeforeRelease=secondSettled;release();const response=await pending,body=await response.json();await Promise.all([closing,secondClosing]);
 assert.equal(abortedAtClose,true,"close must synchronously abort the active generation");assert.equal(closing,secondClosing,"every close call must return the one shared teardown promise");assert.equal(secondSettledBeforeRelease,false,"repeated close must not resolve before active teardown settles");assert.equal(response.status,500);assert.deepEqual(body,{code:"CANCELLED",authority:"NONE"});assert.equal(externalCalls,0);assert.equal(projectCalls,0,"late completion after close must never restore preview");
});

test("server close failure waits for active generation settlement before rejecting shared teardown",async t=>{
 let handler,release,enteredResolve,destroys=0;const entered=new Promise(resolve=>{enteredResolve=resolve}),server=new EventEmitter(),token=Buffer.from("fixture-secret"),github={provider:"github-rest-v1",apiOrigin:"https://api.github.com",owner:"nousresearch",repository:"hermes-agent",pullNumber:7,apiVersion:"2022-11-28",credentialFd:9,permissionAttestation:"READ_ONLY_DECLARED",deadlines:{connectMs:1000,headerMs:2000,bodyMs:3000,overallMs:5000},limits:{perPage:100,maxPages:10,maxRows:1000,bodyBytes:1048576,aggregateBytes:8388608}};
 server.listen=(_port,_host,callback)=>queueMicrotask(callback);server.address=()=>({port:32123});server.close=callback=>queueMicrotask(()=>callback(Object.assign(new Error("server-close-failed"),{code:"EIO"})));
 const invoke=(method,url,headers)=>new Promise((resolve,reject)=>{const request=new EventEmitter();Object.assign(request,{method,url,headers,socket:{localAddress:"127.0.0.1",remoteAddress:"127.0.0.1"}});const response={status:0,headers:null,writeHead(status,responseHeaders){this.status=status;this.headers=responseHeaders},end(body=""){resolve({status:this.status,body:String(body)})}};Promise.resolve(handler(request,response)).catch(reject);queueMicrotask(()=>request.emit("end"));});
 const {service}=await productionHarness(t,{skipCleanup:true,github,run:async(_config,stage)=>{enteredResolve(stage.signal);return new Promise(resolve=>{release=()=>resolve({state:"INDETERMINATE",reason:"CANCELLED",results:[]})})},adapters:{acquireGithubCredential:async()=>token,createGithubTransport:()=>({request:async()=>{},destroy(){destroys++;throw new Error("transport-destroy-failed")}}),createGithubEngine:()=>({observe:async()=>{throw new Error("unreachable")}}),createHttpServer:callback=>{handler=callback;return server}}});
 const page=await invoke("GET","/",{host:service.authority}),cap=/data-session="([0-9a-f]{64})"/.exec(page.body)[1],headers={host:service.authority,origin:`http://${service.authority}`,"content-type":"application/json","x-workhub-aca-session":cap};
 const observing=invoke("POST","/api/observe",headers),signal=await entered,closing=service.close();let closeState="pending";closing.then(()=>{closeState="resolved"},()=>{closeState="rejected"});await new Promise(resolve=>setTimeout(resolve,20));
 assert.equal(signal.aborted,true);assert.equal(closeState,"pending","server close failure must not settle teardown before the active generation");release();const response=await observing;assert.equal(response.status,500);await assert.rejects(closing,/server-close-failed/);assert.equal(closeState,"rejected");assert.equal(destroys,1);assert.equal(token.every(byte=>byte===0),true,"credential bytes must be wiped even when transport destruction throws");
});
...
test("live startup failure after credential acquisition destroys transport and wipes retained token before listen",async t=>{
 ...
 await assert.rejects(()=>productionHarness(t,{github,adapters:{acquireGithubCredential:async(_config,{readFd,closeFd})=>{reads++;await readFd(9);await closeFd(9);return token},readCredentialFd:async()=>Buffer.from("fixture-secret\n"),closeCredentialFd:async()=>{closes++},createGithubTransport:()=>({request:async()=>{},destroy(){destroys++;throw new Error("transport-destroy-failed")}}),createGithubEngine:()=>({observe:async()=>{}}),createDurableJournal:async()=>{throw new Error("JOURNAL_STARTUP_FAILED")}}}),/JOURNAL_STARTUP_FAILED/);
 assert.equal(reads,1);assert.equal(closes,1);assert.equal(destroys,1);assert.equal(token.every(byte=>byte===0),true);
});

test("connector construction cleanup preserves startup errors and awaits sync or async destruction",async t=>{
 const github={provider:"github-rest-v1",apiOrigin:"https://api.github.com",owner:"nousresearch",repository:"hermes-agent",pullNumber:7,apiVersion:"2022-11-28",credentialFd:9,permissionAttestation:"READ_ONLY_DECLARED",deadlines:{connectMs:1000,headerMs:2000,bodyMs:3000,overallMs:5000},limits:{perPage:100,maxPages:10,maxRows:1000,bodyBytes:1048576,aggregateBytes:8388608}};
 for(const asynchronous of [false,true]){let destroys=0;const token=Buffer.from("fixture-secret"),destroy=()=>{destroys++;if(asynchronous)return Promise.reject(new Error("destroy-async"));throw new Error("destroy-sync")};await assert.rejects(()=>productionHarness(t,{github,adapters:{acquireGithubCredential:async()=>token,createGithubTransport:()=>({request:async()=>{},destroy}),createGithubEngine:()=>{throw new Error("engine-startup")}}}),/engine-startup/);assert.equal(destroys,1);assert.equal(token.every(byte=>byte===0),true)}
});

test("successful close awaits and propagates asynchronous cleanup failure after wiping credentials",async t=>{
 const github={provider:"github-rest-v1",apiOrigin:"https://api.github.com",owner:"nousresearch",repository:"hermes-agent",pullNumber:7,apiVersion:"2022-11-28",credentialFd:9,permissionAttestation:"READ_ONLY_DECLARED",deadlines:{connectMs:1000,headerMs:2000,bodyMs:3000,overallMs:5000},limits:{perPage:100,maxPages:10,maxRows:1000,bodyBytes:1048576,aggregateBytes:8388608}},token=Buffer.from("fixture-secret");let destroys=0;
 const {service}=await productionHarness(t,{skipCleanup:true,github,adapters:{acquireGithubCredential:async()=>token,createGithubTransport:()=>({request:async()=>{},async destroy(){destroys++;throw new Error("destroy-async-close")}}),createGithubEngine:()=>({observe:async()=>{throw new Error("unreachable")}})}});await assert.rejects(service.close(),/destroy-async-close/);assert.equal(destroys,1);assert.equal(token.every(byte=>byte===0),true);
});
...
test("bind/listen failure destroys the initialized live connector and emits no service",async t=>{
 ...
 await assert.rejects(()=>productionHarness(t,{github,adapters:{acquireGithubCredential:async()=>token,createGithubTransport:()=>({request:async()=>{},destroy(){destroys++;throw new Error("transport-destroy-failed")}}),createGithubEngine:()=>({observe:async()=>{}}),createHttpServer:()=>server}}),/EADDRINUSE|private-bind-path/);
 assert.equal(destroys,1);assert.equal(token.every(byte=>byte===0),true);
});
