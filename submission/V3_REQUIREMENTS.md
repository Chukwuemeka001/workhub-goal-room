...
Final `qa:submission:v3` is an exact-package qualification gate: it accepts only a metadata child whose parent is the bound package commit and whose child diff contains only `submission/V3_AUDIT.md` and `submission/package-manifest.json`. The separately named `--prebind` mode is a non-qualifying construction check and cannot emit final PASS. The validator is not a generic validator for arbitrary packages.
